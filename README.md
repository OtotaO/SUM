# SUM: source review and checkable transformations
...
**SUM helps people reshape text, inspect changes against the original source, and share a checkable review.** Its long-term aim is a knowledge distiller with useful levels of detail and perspective over retained sources. Today the browser workbench supports source/rewrite review and receipt-aware export; the CLI and MCP add model-based research measurements and offline verification.

A signed receipt identifies the bytes and measurements an issuer attested. It does not prove that meaning survived or that the issuer is a trusted organization. Keep source review, model measurements, statistical assumptions and cryptographic checks separate.
...
A render receipt verifies the *render attestation* (issuer signed this tome, these triples, this slider position, this model, at this time). It does not verify the truth of the tome's content. The slider bench measures a named preservation proxy on a particular corpus; it does not establish factual truth either. See [`docs/RENDER_RECEIPT_FORMAT.md`](docs/RENDER_RECEIPT_FORMAT.md) §5 for the explicit trust scope.
...
`pairs.jsonl` is your own data, one `{"source": "…", "rendering": "…"}` per line. Aim for **n ≥ ~32**: small samples often produce a vacuous bound near 1.0. New receipts default to descriptive batch scope; a confidence interpretation for future expected proxy loss requires independent calibration draws from the target population, a fixed policy and no unaccounted calibration reuse. Sample size alone establishes none of those conditions. (`--source FILE --rendering FILE` mints from a single pair, which is exactly that vacuous case.) `--gen-key` writes a fresh keypair plus JWKS; the private half is mode 0600 and never leaves your machine.
...
Historical slider preservation numbers describe their named corpora and judges. The NLI audit examined embedding-flagged cases; it cannot establish that high-scoring cases contain no missed losses. The live benchmark runner is implemented, but committed evidence for the current production generation loop and independent human validation remain open. See [`docs/SLIDER_CONTRACT.md`](docs/SLIDER_CONTRACT.md) and [`docs/BENCH_HARDENING_FROM_QCVV.md`](docs/BENCH_HARDENING_FROM_QCVV.md).
...
`sum-mcp` exposes the bundle verbs (`extract`, `attest`, `verify`, `inspect`, `render`, `schema`) plus the meaning layer (`verify_receipt`, `meaning_diff`, `depth_frontier`, `mint_meaning_receipt`, `mint_chain_receipt` — BYO private key only; the server never generates or stores keys) as MCP tools. Bundles attested via MCP verify byte-identically through the CLI / Node / browser verifiers — same canonical codec — and receipt verdicts carry the same honesty fields (`proxy_caveat`, `budget_scope`) the CLI prints. See [`docs/MCP_INTEGRATION.md`](docs/MCP_INTEGRATION.md) for the full client wiring and the measured concurrency story.
