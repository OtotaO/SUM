...
- **Path 2 multi-LLM cross-family corroboration** (pinned in [`Tests/research/test_sheaf_path2_multi_llm_compare.py`](../Tests/research/test_sheaf_path2_multi_llm_compare.py); per-model `bench_digest`s `7b364fc6…cc4b75e` for `gpt-4o-mini-2024-07-18` and `d0f9f175…2f6f7` for `claude-haiku-4-5-20251001`; receipt at [`fixtures/bench_receipts/path2_multi_llm_compare_2026-05-05.json`](../fixtures/bench_receipts/path2_multi_llm_compare_2026-05-05.json); Anthropic snapshot at [`fixtures/bench_renders/path2_claude-haiku-4-5-20251001.json`](../fixtures/bench_renders/path2_claude-haiku-4-5-20251001.json)). **Joint finding: `STRUCTURAL_GAP_ALL_MODELS_LOSE`** (gpt-4o-mini Δ = −0.021; claude-haiku-4.5 Δ = −0.032; Δ-spread between models 0.011). Two independent LLM families with different training corpora, architectures, and adversarial-prompt-following styles produce the same directional verdict with similar magnitude. The §4.7.2 synthetic-vs-real gap is structural rather than gpt-4o-mini-specific. **Honest scope:** two LLM families is a thin sample — enough to falsify model-independence (it didn't), enough to weakly support a structural reading. v0.4+ candidate: open-weights extension (Llama-3.x / Mistral) to broaden beyond proprietary frontier APIs. See preprint §4.7.3 for the full per-model table.
