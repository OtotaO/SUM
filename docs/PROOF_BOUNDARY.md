...
The cross-runtime canonicalisation rule (JCS normalises integer-valued floats — `1.0` → `1`, `-0` → `0` — per RFC 8785 §3.2.2.3) is byte-stable across the Erdtman `canonicalize` (TypeScript, what the Worker signs with) and `jcs` (Python). **Scope correction (2026-06-09):** a pre-publication audit found the Python encoder previously fell back to `repr(f)`, which diverges from the ECMAScript `Number::toString` form mandated by RFC 8785 at the exponential-notation boundary (`density=1e-6` → Python `1e-06` vs JS `0.000001`) — a *reachable* divergence, since the render `density` slider is passed through unsnapped, that made a Worker-signed receipt fail Python verification. `jcs._ecmascript_number_to_string` now emits the exact ECMAScript form; byte-identity is verified across **all 24 float cases (incl. the full sub-1e-4 band) + the slider-bearing payloads** against the real Erdtman library in `Tests/test_jcs_float_cross_runtime.py`, and across the float-free integer/string/unicode fixtures in `scripts/verify_jcs_byte_identity.py`. Integers beyond 2⁵³ would diverge (Python-exact vs JS-Number-lossy) but are never on the wire as numbers (the Gödel state integer rides as a string; receipt micro-units ≤ 1e6).
...
**Cross-runtime negative-path proof (Phase E.1 v0.9.B + v0.9.C, complete):** The shared receipt-fixture set under [`fixtures/render_receipts/`](../fixtures/render_receipts/) — 20 runtime-neutral JSON receipt cases (2 positive controls + 10 tampered-field variants + 8 negative-path / revocation / algorithm-downgrade / forward-compat variants), plus the at-capture JWKS — is consumed unchanged by both:
...
- **Path 2 — real-LLM-rendered adversarial bench** (pinned in [`Tests/research/test_sheaf_path2_v3.py`](../Tests/research/test_sheaf_path2_v3.py); `bench_digest 7b364fc6…cc4b75e`; receipt at [`fixtures/bench_receipts/path2_v3_bench_2026-05-05.json`](../fixtures/bench_receipts/path2_v3_bench_2026-05-05.json); LLM render snapshot at [`fixtures/bench_renders/path2_seed_long_paragraphs.json`](../fixtures/bench_renders/path2_seed_long_paragraphs.json) captured against `gpt-4o-mini-2024-07-18`). **Verdict: `HYBRID_LOSES_TO_BASELINE_ON_REAL_LLM`, Δ = −0.021 trusted-mean.** The synthetic-bench WIN (§4.7.1, Δ = +0.043) does NOT generalise to real LLM hallucinations; all detectors weaken substantially (B2 mean: synthetic 0.833 → real 0.660). The synthetic-vs-real gap is itself the load-bearing finding: synthetic A1/A2/A4 perturbations have a structural property real LLM perturbations don't share, which inflates both the strengths and weaknesses of B2 in ways that make the hybrid's complementary-signal composition look stronger than it is on naturalistic prose. See preprint §4.7.2 for the full per-cell table + v0.4 candidate directions.

- **Path 2 multi-LLM cross-family corroboration** (pinned in [`Tests/research/test_sheaf_path2_multi_llm_compare.py`](../Tests/research/test_sheaf_path2_multi_llm_compare.py); receipt at [`fixtures/bench_receipts/path2_multi_llm_compare_2026-05-05.json`](../fixtures/bench_receipts/path2_multi_llm_compare_2026-05-05.json)). **Joint finding on `seed_long_paragraphs`: `STRUCTURAL_GAP_NO_MODEL_BEATS`** across **six** organisationally distinct LLM lineages: OpenAI gpt-4o-mini (Δ = −0.021), Anthropic claude-haiku-4.5 (Δ = −0.032), Meta Llama-3.3-70B (Δ = −0.047), Alibaba Qwen3.6-35B-A3B (Δ = +0.003), DeepSeek V3-0324 (Δ = +0.018), Google Gemma-3-27B (Δ = −0.028). Four LOSE; two TIE; **zero BEAT**. The four open-weights members route through Hugging Face Inference Providers via the OpenAI-compatible router; the closed pair use vendor APIs. Per-model `bench_digest`s `7b364fc6…cc4b75e`, `d0f9f175…2f6f7`, `f1c17c3e…aac29b31`, `23da3ecb…461b8ea2`, `619a413f…2fe22c9f`, `fe76913e…9318b9b5` (in the receipt order above). On `seed_long_paragraphs` specifically, the synthetic-vs-real gap survives extension from closed to open weights. **Honest scope:** six lineages × one corpus is meaningfully wider than a "thin sample" but still bounded — see §4.7.4 cross-corpus extension below for the broader picture. See preprint §4.7.3 for the full per-model table and texture analysis.

- **Path 2 cross-corpus extension (§4.7.4)** (pinned in [`Tests/research/test_sheaf_path2_cross_corpus.py`](../Tests/research/test_sheaf_path2_cross_corpus.py); aggregate receipt at [`fixtures/bench_receipts/path2_cross_corpus_compare_2026-05-06.json`](../fixtures/bench_receipts/path2_cross_corpus_compare_2026-05-06.json); per-corpus receipts at `path2_multi_llm_compare_seed_long_paragraphs_2026-05-06.json`, `..._seed_paragraphs_...`, `..._seed_news_briefs_...`). **Joint finding: `CROSS_CORPUS_VERDICTS_DIVERGE`** mechanically (per-corpus joint findings differ), but the divergence is a *measurement artifact at small $n$*, not a substantive model-dependence claim. Across four corpora: `seed_long_paragraphs` (16 docs, encyclopedic, n=6 with claude) → `STRUCTURAL_GAP_NO_MODEL_BEATS`; `seed_paragraphs` (8 docs, the small-$n$ canary, n=5) → `MIXED_VERDICTS_MODEL_DEPENDENT` (one BEATS cell — gpt-4o-mini Δ=+0.032, right at the +0.030 threshold); **`seed_paragraphs_16` (16 docs, same encyclopedic style as seed_paragraphs but doubled $n$, n=5) → `STRUCTURAL_GAP_NO_MODEL_BEATS`** (gpt-4o-mini's BEATS verdict flips to TIES at Δ=−0.013, directly confirming the extremal-Goodhart hypothesis); `seed_news_briefs` (16 docs, news-wire, n=5) → `STRUCTURAL_GAP_NO_MODEL_BEATS`. Across the jagged 21-cell matrix (6 + 5 + 5 + 5): **1 BEATS, 10 TIES, 10 LOSES** — and the lone BEATS cell is now *explained* (small-$n$ noise at the threshold), not unresolved. Honest top-level reading: at controlled sample sizes ($n \geq 16$), the hybrid does not BEAT B2 on real LLM perturbations across any of the LLM families tested. See preprint §4.7.4 / §4.7.4.1 for the full per-(corpus, model) table and the four-point honest reading. The §4.7.3 finding is corpus-specific rather than corpus-independent; the §4.7.4 narrative reads honestly as *the hybrid does not consistently beat baseline on real-LLM perturbations across LLM families × corpora, but isolated cells can produce positive Δ at the threshold*. The synthetic-bench WIN (Δ=+0.043, §4.7.1) still sits substantially above the lone real-LLM BEATS cell (Δ=+0.032); the synthetic-vs-real magnitude gap is real even where the verdict-class gap narrows. See preprint §4.7.4 for the full per-(corpus, model) table and the four-point honest reading.

- **Sprint-7.5 recovery-experiment digests** (all pinned in [`Tests/research/test_recovery_experiment_digests.py`](../Tests/research/test_recovery_experiment_digests.py); **all four pinned by byte-digest** post-v0.3 deterministic-BLAS fix):
  - `a7965803…6c2003` (Apple Accelerate; OpenBLAS x86_64 — CI Linux + Modal Py 3.10/3.12) **OR** `7fac833a…3bcb97` (OpenBLAS arm64 — miniforge / conda-forge numpy on Apple Silicon) — Borda(v3.2_only, B2) loses (Δ ≈ −0.025) — **two-architecture stable digest pin**. Pre-v0.3 was shape-pinned because cochain-only Borda fusion was sensitive to BLAS thread-pool-size variance at numpy-import time on Apple Accelerate. The v0.3 fix: [`scripts/research/_deterministic_blas.py`](../scripts/research/_deterministic_blas.py) sets `VECLIB_MAXIMUM_THREADS=1` + `OPENBLAS_NUM_THREADS=1` + `MKL_NUM_THREADS=1` + `OMP_NUM_THREADS=1` etc. via `os.environ.setdefault` at module-import time, BEFORE numpy is imported. Each Sprint-7.5 bench imports the helper as its first non-future import. The pin test runs the bench in a fresh subprocess (because pytest itself imports numpy via Hypothesis BEFORE test functions execute, making in-process env-var setdefault a no-op). The thread-pool fix nailed Apple Accelerate to a single outcome and incidentally agrees with OpenBLAS x86_64; OpenBLAS arm64 produces a *different* stable outcome because of NEON-vs-AVX2 SIMD floating-point differences (~1 ULP per `lstsq` compounded across 200 SGD epochs) — both are documented as accepted digests in [`Tests/research/test_recovery_experiment_digests.py::test_hybrid_comparison_digest_pinned`](../Tests/research/test_recovery_experiment_digests.py); the substantive `BORDA_LOSES_TO_B2` verdict and Δ ≈ −0.025 are identical across both.
  - `ddf41484…001f59c3` — predicate-perturbation training fails to lift A2 — **byte-digest pin** (post-v0.2-Issue-3 refactor; verified 5× in fresh procs on operator + 3-environment Modal v6 ALL_MATCH, including Python 3.10 ↔ Python 3.12). The pre-refactor bench used a local copy of the v2 training loop and was Python-version-sensitive (`aa34b6e8…` Py 3.10 vs `8638253903…` Py 3.12). The Issue 3 refactor extended production `train_restriction_maps` with `n_predicate_negatives_per_positive` parameter and replaced the local copy; single training-loop source eliminates the cross-version drift. The substantive structural finding (cochain blindness to entity-set-preserving perturbations) is invariant.
  - `7025436f…fd4fa` — per-triple integration lifts A2 from 0.500 to 0.671 — **byte-digest pin** (verified 5× in fresh procs; 3-environment Modal v6 MATCH on operator + Modal Py 3.10 + Modal Py 3.12).
  - `dc6e0260…343ce` — complementary Borda(v3.2 + per-triple, B2) WINS (`HYBRID_BEATS_BASELINE`) — **byte-digest pin** (verified 5× in fresh procs; 3-environment Modal v6 MATCH; per-triple V magnitude breaks LAPACK ties cleanly so this fusion IS byte-stable, in contrast to `hybrid_comparison`).
...
**Why the substrate needs continuous enforcement.** The substrate's reproducibility property is not a proved invariant of the code; it is an *empirically-enforced* property that the project re-verifies on every PR. Without continuous enforcement — the digest pin tests, the cross-machine matrix, the dict-order regression test — the property would drift toward parasitism in the same way mutualism drifts when sanctioning costs are not paid (Sachs et al., *QRB* 79:135, 2004 give the biological-mutualism analogue). PR #160 is a worked example: the "Phase 1 / Phase 2 same-process digest contamination" was *misdiagnosed* in the prior session as BLAS / asyncio state pollution; the actual root cause was dict iteration order (in-memory snapshots have corpus order; cached snapshots have alphabetical order via `json.dumps(..., sort_keys=True)`). The mismatch silently changed the trained sheaf's edge-ordering, and hence the digest, even though the snapshot bytes were equivalent. The bug was caught only because the digest pin and a probe-style investigation surfaced it. **The reproducibility primitive enforces what could otherwise drift unobserved.** A new regression test (`test_path2_v3_bench_invariant_to_snapshot_dict_order`) now asserts Phase 2 is invariant to snapshot dict order; the existing pins were already against alphabetical/cached order, so they continued to hold. The lesson generalises: every `bench_digest` pin in this section is a sanction against a specific failure mode, and the substrate's claim is alive only as long as the sanctions remain in CI.
...
2. **Cross-machine witness** (Sprint 7.5 — measured; v0.2 expanded to three environments × three benches; **v0.3 expanded to four benches × three environments, all MATCH**). Four load-bearing benches each reproduce byte-for-byte across three distinct LAPACK environments:
   ...
   The `predicate_negatives` cross-version digest stability was a v0.2 latent issue (Issue 3) — the bench previously used a local v2-training-loop copy whose SGD trajectory was Python-version-sensitive (Modal Py 3.10 produced `aa34b6e8…`; Modal Py 3.12 produced `8638253903…`). The v0.2 latent-fix refactored the bench to call production `train_restriction_maps` with a new `n_predicate_negatives_per_positive` parameter, eliminating the cross-version drift; the post-refactor digest `ddf41484…` now matches across all three environments.
