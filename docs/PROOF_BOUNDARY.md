...
- **Sprint-7.5 recovery-experiment digests** (all pinned in [`Tests/research/test_recovery_experiment_digests.py`](../Tests/research/test_recovery_experiment_digests.py); three of four pinned by byte-digest, one pinned by behavior-shape):
  - `a7965803…6c2003` (one of two stable outcomes; the other is `7fac833a…3bcb97`) — Borda(v3.2_only, B2) loses (Δ ≈ −0.025) — **shape pin only** (verdict label `BORDA_LOSES_TO_B2` + Δ ∈ [−0.10, −0.02]), NOT byte-digest. The v0.2 latent-fix arc tried two layers of quantization (rank-key in `_ranks`; per-pair-score at storage time) but neither fully absorbs LAPACK-jitter cell-AUC sensitivity in this specific bench: cochain-only Borda fusion (no per-rendered-triple V magnitude to break ties) lands at quantization-bucket boundaries on some cells. Same-machine reruns produce two stable digests (~62% / ~38% across 8 fresh-proc runs); both are equally valid `BORDA_LOSES_TO_B2` outcomes. The complementary-hybrid bench (which DOES include per-triple V) is byte-stable, confirming the per-triple magnitude is the load-bearing tie-breaker. The two-layer quantization is retained because it statistically improves stability. v0.3+ candidate fixes: replace `np.linalg.lstsq` with deterministic SVD-based pinv in `harmonic_extension`; OR set `OPENBLAS_NUM_THREADS=1` at process entry (env-dependent); OR quantize at the AUC level rather than score level.
