...
- **Sprint-7.5 recovery-experiment digests** (all pinned in [`Tests/research/test_recovery_experiment_digests.py`](../Tests/research/test_recovery_experiment_digests.py); three of four pinned by byte-digest, one pinned by behavior-shape):
  - `a7965803…6c2003` — Borda(v3.2_only, B2) loses (Δ=−0.025) — byte-digest pin (cross-Python-version stable; uses production training)
  - `aa34b6e8…c866e7` — predicate-perturbation training fails to lift A2 — **shape pin only** (verdict label + A2 cells at 0.500), NOT byte-digest. The bench uses a local copy of the v2 training loop and is Python-version-sensitive: digest reproduces on Python 3.10 (operator + Modal) but diverges on Python 3.12 (CI). The substantive structural finding (cochain blindness to entity-set-preserving perturbations) holds invariant. Cross-Python-version digest reproducibility is a v0.2 follow-up (would require upstreaming the predicate-negative sampler into production `train_restriction_maps`).
  - `7025436f…fd4fa` — per-triple integration lifts A2 from 0.500 to 0.671 — byte-digest pin
  - `dc6e0260…343ce` — complementary Borda(v3.2 + per-triple, B2) WINS (HYBRID_BEATS_BASELINE) — byte-digest pin (cross-machine MATCH on Modal x86_64)
