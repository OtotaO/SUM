...
- **Path 2 multi-LLM cross-family corroboration** (pinned in [`Tests/research/test_sheaf_path2_multi_llm_compare.py`](../Tests/research/test_sheaf_path2_multi_llm_compare.py); receipt at [`fixtures/bench_receipts/path2_multi_llm_compare_2026-05-05.json`](../fixtures/bench_receipts/path2_multi_llm_compare_2026-05-05.json)). **Joint finding: `STRUCTURAL_GAP_NO_MODEL_BEATS`** across **six** organisationally distinct LLM lineages: OpenAI gpt-4o-mini (Δ = −0.021), Anthropic claude-haiku-4.5 (Δ = −0.032), Meta Llama-3.3-70B (Δ = −0.047), Alibaba Qwen3.6-35B-A3B (Δ = +0.003), DeepSeek V3-0324 (Δ = +0.018), Google Gemma-3-27B (Δ = −0.028). Four LOSE; two TIE; **zero BEAT**. The four open-weights members route through Hugging Face Inference Providers via the OpenAI-compatible router; the closed pair use vendor APIs. Per-model `bench_digest`s `7b364fc6…cc4b75e`, `d0f9f175…2f6f7`, `f1c17c3e…aac29b31`, `23da3ecb…461b8ea2`, `619a413f…2fe22c9f`, `fe76913e…0318b9b5` (in the receipt order above). The synthetic-vs-real gap is structural rather than gpt-4o-mini-specific *and* survives extension from closed to open weights. **Honest scope:** six lineages × one corpus is meaningfully wider than a "thin sample" but still bounded; the load-bearing claim — *no LLM family in the sample makes the hybrid BEAT the baseline* — is robust, the precise per-model Δ values are not. See preprint §4.7.3 for the full per-model table and texture analysis.
