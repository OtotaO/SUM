...
**Boundary on the seed_v2 result.** Still corpus-bounded: the §2.5 closure on seed_v1 + seed_v2 covers single-fact SVO and 20-doc difficulty-corpus shapes. `seed_long_paragraphs.json` (16 hand-authored multi-paragraph docs, 9–24 triples each) remains unmeasured under the intervention. The seed_v2 result establishes that the intervention pattern is **structurally right** across difficulty-pattern variation; whether it holds on multi-paragraph multi-fact docs is the next measurement when budget allows.
...
| Round-trip conservation (LLM narrative prose, full loop) | **Closed on `seed_v1`; scales to `seed_v2`** | Baseline (2026-04-19): drift = 107.75 %, exact-match recall = 0.12. Combined intervention on `seed_v1` (50 docs, single-fact SVO): **drift = 0.00 %, recall = 1.0000, 50 / 50 docs full recall**. Combined intervention on `seed_v2` (20 docs, 7 difficulty parse patterns + multi-fact): **drift = 5.00 %, recall = 0.9750, 19 / 20 docs full recall** — the single failure is an LLM-source-extraction artifact, not an intervention failure. `seed_long_paragraphs` (16 multi-paragraph docs) remains unmeasured. Per-ablation breakdowns + receipt schemas in §2.5. |
