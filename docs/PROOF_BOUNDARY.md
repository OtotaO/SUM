...
| `input_kind="prose"` | 42.86 % | **54.00 %** | **60.00 %** | `empirical-benchmark` | Sieve re-extraction of the system's own canonical-template output (`generate_canonical` → `extract_triplets`) loses axioms on every corpus; the drift is a direct function of the fraction of sentences spaCy parses atypically or that the sieve lemmatizer normalises away. |
...
|---|---|---|
| Tome → Tag (extraction) | **F1 = 1.000** on seed_v1, **F1 = 0.634** on seed_v2 | seed_v1: 50/50 simple SVO. seed_v2 (difficulty corpus, 20 docs × 7 parse patterns — apposition, passive voice, relative clause, conjunction, negation, hedging, complex PP): **precision 0.867, recall 0.500, TP=13/26, predicted=15**. The 0.634 number is the honest empirical ceiling on real-prose constructions after the negation suppression fix; seed_v2's per-doc failures are tabulated in `scripts/bench/corpora/seed_v2.json`. Remaining sieve failure modes (reducing RECALL, not precision): passive voice reverses subject/object semantics (doc_007, doc_008); partial-passive returns empty (doc_009); relative clauses drop the subordinate fact (doc_010-012); apposition drops the "X be Y" fact (doc_004-006); compound subject/object drops all but the first conjunct (doc_013-015); prepositional-complement verbs return empty (doc_020). Negation (doc_016, doc_017) and modal hedging (doc_018) are now intentionally suppressed by the sieve — emitting a bare SVO triple from a negated sentence would ship an inverted false positive into the Gödel state. Suppression is strictly preferable to polarity-flipped assertion. |
