...
- **Path 2 cross-corpus extension (§4.7.4)** (pinned in [`Tests/research/test_sheaf_path2_cross_corpus.py`](../Tests/research/test_sheaf_path2_cross_corpus.py); aggregate receipt at [`fixtures/bench_receipts/path2_cross_corpus_compare_2026-05-06.json`](../fixtures/bench_receipts/path2_cross_corpus_compare_2026-05-06.json); per-corpus receipts at `path2_multi_llm_compare_seed_long_paragraphs_2026-05-06.json`, `..._seed_paragraphs_...`, `..._seed_news_briefs_...`). **Joint finding: `CROSS_CORPUS_VERDICTS_DIVERGE`** across three corpora: `seed_long_paragraphs` (16 docs, encyclopedic, n=6 with claude) → `STRUCTURAL_GAP_NO_MODEL_BEATS`; `seed_paragraphs` (8 docs, encyclopedic, shorter, n=5 — Anthropic key unavailable for the extension) → `MIXED_VERDICTS_MODEL_DEPENDENT` (one BEATS cell — gpt-4o-mini Δ=+0.032, right at the +0.030 threshold); `seed_news_briefs` (16 docs, news-wire, n=5) → `STRUCTURAL_GAP_NO_MODEL_BEATS`. Across the jagged 16-cell matrix (6 + 5 + 5): **1 BEATS, 8 TIES, 7 LOSES**. The §4.7.3 finding is corpus-specific rather than corpus-independent; the §4.7.4 narrative reads honestly as *the hybrid does not consistently beat baseline on real-LLM perturbations across LLM families × corpora, but isolated cells can produce positive Δ at the threshold*. The synthetic-bench WIN (Δ=+0.043, §4.7.1) still sits substantially above the lone real-LLM BEATS cell (Δ=+0.032); the synthetic-vs-real magnitude gap is real even where the verdict-class gap narrows. See preprint §4.7.4 for the full per-(corpus, model) table and the four-point honest reading.
