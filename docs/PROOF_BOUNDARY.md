...
| `input_kind="prose"` | 42.86 % | **54.00 %** | **58.82 %** | `empirical-benchmark` | Sieve re-extraction of the system's own canonical-template output (`generate_canonical` → `extract_triplets`) loses axioms on every corpus; the drift is a direct function of the fraction of sentences spaCy parses atypically or that the sieve lemmatizer normalises away. |
...
|---|---|---|
| Tome → Tag (extraction) | **F1 = 1.000** on seed_v1, **F1 = 0.605** on seed_v2 | seed_v1: 50/50 simple SVO. seed_v2 (difficulty corpus, 20 docs × 7 parse patterns — apposition, passive voice, relative clause, conjunction, negation, hedging, complex PP): precision 0.76, recall 0.50, TP=13/26. The 0.605 number is the honest empirical ceiling on real-prose constructions; seed_v2's per-doc failures are tabulated in `scripts/bench/corpora/seed_v2.json`. Non-SVO patterns that specifically break the sieve: passive voice reverses subject/object semantics; negation strips the "not" and emits the WRONG fact as a false positive; hedging/modal produces empty extraction; multi-clause drops the subordinate clause. These are genuine product-relevant failure modes and are now load-bearing in the published F1 number. |
