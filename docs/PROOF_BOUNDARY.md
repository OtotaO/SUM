...
- **Sprint-7.5 recovery-experiment digests** (all pinned in [`Tests/research/test_recovery_experiment_digests.py`](../Tests/research/test_recovery_experiment_digests.py); two of four pinned by byte-digest, two pinned by behavior-shape):
  - `a7965803…6c2003` — Borda(v3.2_only, B2) loses (Δ≈−0.025) — **shape pin only** (verdict label = `BORDA_LOSES_TO_B2` + Δ ∈ [−0.10, −0.02]), NOT byte-digest. Borda fusion of only the cochain V channel (no per-rendered-triple V magnitude to break ties) is sensitive to LAPACK-jitter rank-shuffles at the per-pair score level — quantization to 3 decimals on AUC doesn't always absorb the resulting cell-AUC shift. Verified on operator's machine: 5 fresh-proc runs produced two digests (`a7965803…` 2/3 and `7fac833a…` 1/3); both are valid `BORDA_LOSES_TO_B2` outcomes. The substantive loss-margin claim (Δ ≈ −0.025) is invariant. v0.2 follow-up: add secondary sort key to `borda_fuse` for stable tie-breaking, OR widen quantization to 2 decimals on AUC for cochain-only fusion benches.
  - `aa34b6e8…c866e7` — predicate-perturbation training fails to lift A2 — **shape pin only** (verdict label = `A2_STILL_CHANCE` + A2 cells at 0.500), NOT byte-digest. The bench uses a local copy of the v2 training loop and is Python-version-sensitive: digest reproduces on Python 3.10 (operator + Modal; verified 3× in fresh procs) but diverges on Python 3.12 (CI). The substantive structural finding (cochain blindness to entity-set-preserving perturbations) holds invariant. v0.2 follow-up: upstream the predicate-negative sampler into production `train_restriction_maps`.
  - `7025436f…fd4fa` — per-triple integration lifts A2 from 0.500 to 0.671 — **byte-digest pin** (verified 5× in fresh procs unconditionally; per-rendered-triple V channel adds magnitude that breaks ties cleanly).
  - `dc6e0260…343ce` — complementary Borda(v3.2 + per-triple, B2) WINS (HYBRID_BEATS_BASELINE) — **byte-digest pin** (verified 5× in fresh procs unconditionally; cross-machine MATCH on Modal x86_64).
