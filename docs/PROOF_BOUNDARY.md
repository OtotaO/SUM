...
**Boundary on this result.** `seed_v1` is a 50-document corpus of single-fact SVO sentences (`Alice likes cats.`, `Bob owns a dog.`, etc.). The 1.00 recall is the saturation point for that corpus's complexity, not a universal §2.5 result. Harder corpora — `seed_v2` (20 difficulty-corpus docs across 7 parse patterns: apposition, passive voice, relative clause, conjunction, negation, hedging, complex PP), `seed_long_paragraphs.json` (16 hand-authored multi-paragraph docs, 9–24 triples each) — have not yet been measured under the combined intervention. The `seed_v1` receipt establishes that the **intervention pattern is right**; whether it scales to multi-fact long-doc corpora is the next measurement.
...
| Round-trip conservation (LLM narrative prose, full loop) | **Closed on `seed_v1`** | Baseline (2026-04-19): drift = 107.75 %, exact-match recall = 0.12. Combined intervention (canonical-first generator prompt + constrained-decoding extractor with vocab-pinned `Literal` enums + lemma-exclusion fix, 2026-04-28): **drift = 0.00 %, exact-match recall = 1.0000, p10 recall = 1.00, 50 / 50 docs at full recall**. Boundary: `seed_v1` is single-fact SVO; harder corpora (`seed_v2`, `seed_long_paragraphs`) not yet measured under the intervention. Per-ablation breakdown + receipt schemas (`sum.s25_generator_side.v1`, residual-closure receipt) in §2.5. |
