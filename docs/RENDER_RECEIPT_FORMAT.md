...
The `digital_source_type` field uses the C2PA `digitalSourceType` taxonomy. SUM tracks the [C2PA Specification v2.4](https://spec.c2pa.org/specifications/specifications/2.4/specs/C2PA_Specification.html) (current as of 2026-04). The two values SUM emits — `trainedAlgorithmicMedia` and `algorithmicMedia` — are **unchanged across C2PA 2.2 → 2.4**; the values below remain authoritative.
...
**Deliberate text-on-image-taxonomy mapping.** As of C2PA 2.4 (Apr 2026), the spec remains image/video-first; no formal text-content profile has landed in 2.x. SUM applies the existing AI-vs-deterministic taxonomy to LLM-rendered prose because the *attribution semantics* are identical (was-this-produced-by-a-trained-model) — only the payload mode differs. If C2PA later ships a text-content profile that supersedes this mapping, SUM will mint a new field rather than overload the existing one (forward-compat per `docs/COMPATIBILITY_POLICY.md`).
