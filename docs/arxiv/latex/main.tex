...
\draw[flow] (pair)    -- node[right, tag] {named transform} (loss);
...
\coordinate (c1) at ($(loss.east)+(9mm,0)$);
\coordinate (c2) at ($(stageB.east)+(9mm,0)$);
\draw[side] (loss.east) -- (c1) -- (c2) -- (stageB.east);
\node[tag, right=9.5mm of payload, align=left, text width=32mm]
...
the verifier side needs the issuer. \textbf{Stage A} establishes that the
...
it on 2026-09-05 and all twelve cells of Table~\ref{tab:coverage} matched, as
