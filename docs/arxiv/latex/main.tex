% arXiv submission kit: Paper 1 (cs.CR primary; cs.LG and cs.CL cross-lists)
...
proxy. A signed, offline-verifiable receipt attests a
...
committed integer loss vector --- a third party re-runs the certifier and
reproduces the bound to the bit --- while the proof boundary
...
deployment, never per-document truth and never ``meaning'' itself. We demonstrate on two public benchmark corpora --- BillSum (CC0) and
opus-100 (licence unspecified, raw text not redistributed) --- over each corpus's own
reference outputs rather than model outputs (the mechanism is
producer-indifferent): certified expected
...
translation (opus-100; $n=64$), with 39/64 reference translations scoring
\emph{exactly zero} meaning-loss (under a binary entailment judge at a 0.5
cut) despite near-zero lexical overlap. The thesis is \textbf{attest, don't
detect}: a receipt authenticates a recorded transformation. Its signature
does not extend coverage to subsequent rewrites, which require new linked
receipts.
\end{abstract}
...
First, \textbf{disclosure}: Article 50(2) of the EU AI Act (applicable
2 August 2026) obliges \emph{providers} to mark AI-generated content in a
machine-readable format and to ensure their technical solutions are
``effective, interoperable, robust and reliable as far as this is
technically feasible''; Article 50(4) separately obliges \emph{deployers}
to disclose deep fakes and AI-generated text published to inform the public
on matters of public interest, subject to the Article's exceptions, including
human review or editorial control together with editorial responsibility for
that text. Providers whose systems were placed on the
market before 2 August 2026 have until 2 December 2026 to comply with
Article 50(2), under Regulation (EU) 2026/1744 (Digital Omnibus on AI, in
force 27 July 2026). The associated \emph{Code of Practice on Transparency
of AI-Generated Content} was finalized 10 June 2026~\cite{euaiact}. Second, and far less served,
...
C2PA~\cite{c2pa} supports text manifests, including Unicode-embedded
manifests designed to persist through copying. Its hard bindings cover a
specified text representation, with NFC normalization for unstructured text
(v2.4, Appendix A.8). Those bindings do not themselves quantify what a
...
exactly the rewriting text invites: detectors have repeatedly
failed under paraphrase and multi-step
rewriting~\cite{advparaphrase2025,synthidrobust2025,chainwash2026}
(\S\ref{sec:related}).
...
verifies offline under the trust preconditions below. A receipt authenticates
the recorded transformation; subsequent rewrites require new linked receipts
and their own measurements. The technical contribution is a \emph{composition}: a
...
Bonferroni simultaneous guarantee. (4) Two real, audited demonstrations on public
benchmark corpora and an empirical coverage validation. (5) An explicit threat
...
\emph{issuer}, while the \emph{bound} is attested only by the
...
\textbf{JWKS} (RFC 7517~\cite{rfc7517}). Figure~\ref{fig:chain} draws the chain end to end. The provenance tier
...
\texttt{repr}-based encoder diverges. The measured-bound-tier payloads are
...
\begin{tikzpicture}[
  ...
  box/.style={draw, rounded corners=1.5pt, align=center, inner sep=4pt,
              text width=58mm},
  stage/.style={box, text width=66mm},
  tag/.style={font=\scriptsize\itshape},
  >={Stealth[length=1.8mm]},
  flow/.style={->, thick},
  side/.style={->, thick, dashed},
]
\node[box] (pair) {source $S$, output $T$ of a named transform};
\node[box, below=of pair] (loss) {per-pair losses, \textbf{integer micro}};
\node[box, below=of loss] (payload)
  {payload: $n$, \texttt{method}, \texttt{delta\_micro}, bound,
   \texttt{losses\_hash}, judge, \texttt{not\_covered}, \texttt{disclosure}};
\node[box, below=of payload] (sig)
  {\textbf{JCS}-canonical bytes, signed \textbf{Ed25519},\\
   carried as a detached \textbf{JWS}};
\node[stage, below=6.5mm of sig] (stageA)
  {\textbf{Stage A}\quad any runtime\\[1pt]
   JWS against \textbf{JWKS}; schema gate; header
   invariants; disclosure\\[2pt]
   $\rightarrow$ \emph{authentic + self-disclosing}};
\node[stage, below=of stageA] (stageB)
  {\textbf{Stage B}\quad Python only at present\\[1pt]
   loss-vector hash; re-run the certifier;
   \textbf{exact integer equality}\\[2pt]
   $\rightarrow$ \emph{bound reproduced}};

\draw[flow] (pair)    -- node[right, tag] {named judge (\S\ref{sec:certificate})} (loss);
\draw[flow] (loss)    -- node[right, tag] {certifier (\S\ref{sec:certificate})} (payload);
\draw[flow] (payload) -- node[right, tag] {sign} (sig);
\draw[flow] (sig)     -- node[right, tag] {verify} (stageA);
\draw[flow] (stageA)  -- (stageB);

% the loss vector travels beside the receipt, never inside it
\coordinate (c2) at ($(stageB.east)+(9mm,0)$);
\coordinate (c1) at (c2 |- loss.east);
\draw[side] (loss.east) -- (c1) -- (c2) -- (stageB.east);
\node[tag, anchor=west, align=left, text width=30mm]
  at ($(c2 |- payload.east)+(2.5mm,0)$)
  {\raggedright committed loss vector,\\ supplied side-band\par};

\node[tag, below=2.5mm of stageB, text width=76mm, align=center]
  {Python, Node and the browser all run Stage A on the same bytes.
   No JavaScript port of the bound kernel exists.};
\end{tikzpicture}
\caption{The receipt chain and the two verification stages of
\S\ref{sec:verify}. The issuer derives everything top to bottom; nothing on
the verifier side contacts the issuer at verification time, the public key
having been obtained out of band (\S\ref{sec:threat}). \textbf{Stage A} establishes that the
receipt is authentic and self-disclosing, and runs in any runtime.
\textbf{Stage B} additionally reproduces the bound by exact integer equality
over the committed loss vector, which travels \emph{beside} the receipt rather
than inside it; that replay is well-defined in any runtime
(\S\ref{sec:substrate}) but is implemented only in Python today
(\S\ref{sec:artifact}). Re-deriving the losses from raw text is a third thing
again, and is machine-pinned to the named judge.}
\label{fig:chain}
\end{figure}
...
knob). The shipped default is Hoeffding. Optional \texttt{auto} mode selects
Clopper--Pearson for binary data and Hoeffding otherwise. Both demonstrations
use Hoeffding; their receipt payloads record that method.
...
every runtime'' means authenticity and disclosure: Stage B's exact-integer
replay is \emph{well-defined} in any runtime (\S\ref{sec:substrate}) but is at
present implemented only in Python (\S\ref{sec:artifact}), and the
\emph{meaning recomputation} is machine-pinned in every runtime.
...
declared cohort over only that cohort's losses. The group-conditional
coverage literature (Gibbs, Cherian \& Cand\`es, 2023~\cite{gibbs2023})
addresses the same worst-cohort concern in the conformal-prediction
setting. The marginal and the per-cohort family are
...
opus-100, aggregated OPUS sources of unspecified licence, raw text not
redistributed). Each is committed (a
...
\subsection{Translation (opus-100, licence unspecified)}\label{sec:translation}

First 64 length-aligned EN$\to$FR pairs of opus-100, local multilingual NLI
judge (mDeBERTa-v3-xnli). opus-100 aggregates OPUS sources and its dataset card lists the licence as
``unknown'', so
the raw text is not redistributed here; the committed corpus pointer is
sha256-pinned instead. The committed receipt's \texttt{disclosure} field
records this as a ``mixed-licence corpus'': that wording predates this
characterization and the signed artifact is left byte-identical rather than
re-signed. As with BillSum, the French side is the corpus's own
...
trials, seed 11; Table~\ref{tab:coverage} reports the result.
...
0.6455 vs 0.650; translation 0.4124 vs 0.518), so the shipped Hoeffding default
of \S\ref{sec:certificate} is also the tighter of the two at this $n$; we
report both.
...
correlates only modestly with the pooled summary-level human meaning
composite (Spearman $\rho$ between $0.267$ and $0.291$), the NLI judge replicates at
...
\item \textbf{Signed-but-semantics-disclaiming receipts.}
  ...
  reasonable, or minimal.'' A related boundary appears in deterministic-inference
  verification (EigenAI~\cite{eigenai2026}), where bit-exact re-execution
  reduces verification to a byte-equality check over \emph{what was
  computed}, and output quality is not addressed. Our
...
\item \textbf{The category error, named independently.} A 2026 analysis
  report on AI-agent identity standards~\cite{survey2026} names the same
  category error: its \S5.1, \emph{The Semantic Intent Gap}, calls
  ``the assumption that cryptographic correctness implies semantic
  correctness'' exactly that, and its \S4.4 describes C2PA's contribution
  as output attribution, binding an agent's identity to the content it
  produces. That report poses the error as an \emph{intent-integrity}
  problem for agents, which this paper does not address; we instrument a
  different corner of the same error, the transformation.
\item \textbf{C2PA / Content Credentials}~\cite{c2pa} bind provenance to
  media; C2PA has carried text manifests since 2.3 (2.4 adds
  structured-text blocks). Text manifests use cryptographic content bindings,
  with NFC normalization for unstructured text (v2.4, Appendix A.8); those
  bindings do not measure preservation under substantive rewriting.
  The C2PA Explainer (\S7.2.2) states that provenance information alone
  cannot tell you whether the content is true, accurate or factual.
  ...
  specifically~\cite{synthidrobust2025} and across neural, watermark-based
  and zero-shot detectors by a single training-free paraphrase
  attack~\cite{advparaphrase2025};
  multi-step rewriting drives detection of a diffusion-LM
  watermark~\cite{chainwash2026} from $\sim$88\% to under 5\% in its strongest
  five-hop settings while holding semantic similarity, the exact regime a meaning-preservation certificate
  is built for. We cite
...
\item \textbf{Distribution-free guarantees.} Hoeffding
  ...
  Hoeffding, empirical-Bernstein, and a betting-based e-CRC bound; the nesting
  holds in low-variance and large-sample regimes, and the betting bound is
  strongest under calibration scarcity. That hierarchy is the one
...
determinism is addressed by batch-invariant kernels, while cross-hardware
reproducibility remains open, which is the niche this
...
we draw it from: Seo et al.~\cite{verifyingverifiers2025} identified 117
mislabeled and 159 ambiguous instances among 1,749 collected benchmark
examples, and observed model-ranking changes after correcting labels and
removing ambiguous instances. These results concern their collected sample,
rather than establishing a universal benchmark error rate. A tighter
betting/empirical-Bernstein confidence sequence is a no-wire-change
tightening. Re-enveloping the receipt as a COSE\_Sign1 Signed Statement and
registering it with a transparency service, so the meaning layer composes with
SCITT (\S\ref{sec:related}) rather than duplicating it, is named future work
and is not implemented. The corpora here demonstrate the mechanism; they are not a
...
Every number in \S\ref{sec:billsum}--\S\ref{sec:coverage} is reproducible
from committed bytes; the judge-validity correlations of \S\ref{sec:scope}
come from the calibration cards in \texttt{docs/CALIBRATION\_CARDS.md} and
the benchmark artifacts they name, not from a receipt fixture.
The two receipt sections replay from the committed loss vectors. The synthetic
sweep of \S\ref{sec:coverage} is not a fixture, but it reproduces from the
shipped certifier over the data-generating process stated there ($n=64$ i.i.d.\
Bernoulli draws at each true rate, $2\times 10^{4}$ trials, seed 11); we re-ran
it on 2026-09-05 and all twelve cells of Table~\ref{tab:coverage} matched (one
cell, Hoeffding at $\mathrm{tl}=0.5$, $\delta=.05$, previously printed $0.992$
and is corrected here to the reproducing $0.991$), as
did the $0.958$ joint-coverage figure from
\texttt{Tests/research/test\_group\_conditional.py}. Separately, every
verification claim in \S\ref{sec:verify} is executable by a third party
...
\item \textbf{Source.} \url{https://github.com/OtotaO/SUM}, Apache-2.0.
  ...
  \texttt{swh:1:snp:\allowbreak 503d73855f8b2829ea227865c61729aa16e79d60},
  which captures \texttt{main} at commit \texttt{b64e57b} (origin
  \texttt{swh:1:ori:\allowbreak a7b5385a59a6fb561cbad53ce12da3149439401e}).
...
\newblock \emph{Coalition for Content Provenance and Authenticity, Technical
  ...
  2.3), and the companion \emph{Explainer} \S7.2.2.
