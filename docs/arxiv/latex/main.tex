...
% --- OPERATOR TODO: fill in the real author block before submission. ---
...
stays explicit: it bounds a named proxy \emph{marginally}, under
\emph{exchangeability}, never per-document truth and never ``meaning''
itself. We demonstrate on two public-domain corpora: certified expected
meaning-loss $\le 0.645$ (95\%) for abstractive summarization of US
Congressional bills (BillSum, CC0; $n=64$) and $\le 0.412$ for EN$\to$FR
...
(source $S$, output $T$) pair with sentence units $\mathrm{sent}(\cdot)$,
define
\[
...
\emph{explicitly conditional on it}.

\paragraph{Certificate.}
Let $(S_i,T_i)_{i=1}^n$ be a calibration sample exchangeable with
deployment, $\ell_i=\ell(S_i,T_i)$, preservation $p_i=1-\ell_i\in[0,1]$. By
...
judge. The abstractive bill$\to$summary transform certifies \textbf{expected
meaning-loss $\le 0.6454$ at 95\%} ($n=64$, mean $0.4925$), controlled
...
judge (mDeBERTa-v3-xnli). The translation transform certifies
...
only five distinct values $\{0.0, 0.4, 0.6, 0.8, 1.0\}$ --- 62/64 pairs fall
on that grid --- so ``exactly zero'' means \emph{bidirectional entailment
...
with known ground truth ($n=64$, $2\times 10^{4}$ trials):
...
data with known ground truth ($n=64$, $2\times 10^{4}$ trials).}
\label{tab:coverage}
\end{table}
...
against a 0.95 target. At the receipts' $n=64$ and observed variance,
Hoeffding is the tighter honest estimator than empirical-Bernstein (BillSum
0.645 vs 0.650; translation 0.412 vs 0.518), confirming the a-priori default
...
bounds are over the \emph{named, filtered} calibration corpora under
...
\emph{low} relative to the unfiltered split. The certificate is a
\textbf{batch} primitive: a single document uses a per-document measurement,
not the bound.
...
was preserved (only that a \emph{named proxy} is bounded, marginally, under
exchangeability); or anything about human-vs-AI authorship --- we ship no
...
\item \textbf{Signed-but-semantics-disclaiming receipts.}
  \textbf{AEX}~\cite{aex2026} independently uses JCS + SHA-256 + Ed25519
  signed transformation-receipt chains for LLM APIs but \textbf{explicitly
  ...
  deterministic-inference attestation (EigenAI~\cite{eigenai2026}) and
  signed tool-call receipts~\cite{toolreceipts2026}. Our contribution is
  precisely that uniformly-disclaimed semantic delta, as a finite-sample,
...
\item \textbf{C2PA / Content Credentials}~\cite{c2pa} bind provenance to
  ...
  regime a meaning-preservation certificate is built for.
...
\item \textbf{Distribution-free guarantees.} Hoeffding
  ...
  concurrent conditional-factuality certificates~\cite{condfact2026}. None
  couples its statistical bound to a signed, replayable receipt, and none
  bounds expected meaning-loss of a \emph{transform} under a named judge.
...
Validity rests on exchangeability between calibration and deployment, which
is assumed, not sampled --- a deliberately disclosed boundary common to all
...
the lightweight lineage anchor, though lighter open judges have since been
reported to match or surpass it (e.g.\ Paladin-mini, 2025) --- and any
judge-conditional bound inherits the judge's \emph{label-noise} ceiling, not
merely its accuracy (Verifying the
Verifiers~\cite{verifyingverifiers2025}). A tighter
...
% sources verified"; two entries carry the draft's own "[exact title to
% verify before submission]" note, preserved below.
...
\bibitem{survey2026}
(Survey) {AI}-content identity and provenance; names the ``semantic intent
  gap.''
\newblock arXiv:2604.23280, 2026.
\newblock [exact title to verify before submission]

\bibitem{chainwash2026}
Multi-hop paraphrase removal of {LLM} text watermarks (``Chainwash'').
\newblock arXiv:2605.05503, 2026.
\newblock [exact title to verify before submission]

\bibitem{condfact2026}
Conditional factuality-controlled {LLMs} with generalization certificates.
\newblock arXiv:2603.27403, 2026.

\bibitem{eigenai2026}
{EigenAI}: deterministic-inference attestation.
\newblock arXiv:2602.00182, 2026.

\bibitem{toolreceipts2026}
Tool receipts, not {ZK} proofs: signed tool-call receipts for agents.
...
Robustness assessment and enhancement of text watermarking for {Google's}
  {SynthID}.
\newblock arXiv:2508.20228, 2025.

\bibitem{advparaphrase2025}
Adversarial paraphrasing: universal training-free evasion of {AI}-text
  detectors.
\newblock arXiv:2506.07001 ({NeurIPS} 2025).
...
Verifying the verifiers: label noise in fact-verification benchmarks.
\newblock arXiv:2506.13342, 2025.
