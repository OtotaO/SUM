...
on matters of public interest. Providers whose systems were placed on the
...
0.6455 vs 0.650; translation 0.4124 vs 0.518), so the a-priori default of
\S\ref{sec:certificate} is also the tighter of the two at this $n$; we
report both.
...
\item \textbf{C2PA / Content Credentials}~\cite{c2pa} bind provenance to
  ...
  structured-text blocks), but the text binding is a
  ...
  watermark~\cite{chainwash2026} from $\sim$88\% to under 5\% while holding
  semantic similarity, the exact regime a meaning-preservation certificate
...
\item \textbf{Source.} \url{https://github.com/OtotaO/SUM}, Apache-2.0.
  ...
  which captures \texttt{main} at commit \texttt{996707c}, the state that
  produced this paper (origin
