...
\item \textbf{Distribution-free guarantees.} Hoeffding
  ...
  concurrent conditional-factuality certificates~\cite{condfact2026}. Two
...
\item \textbf{Verifier.} \texttt{pip install "sum-engine[verify]"} installs
  the dependency-light \texttt{sum\_verify} SDK (\texttt{cryptography} and
  \texttt{joserfc} only, no numpy, scipy, or torch);
  \texttt{python -m sum\_verify <receipt>} reproduces the verdicts described
  in \S\ref{sec:verify}. A second, independent implementation verifies the
  same bytes: the JS verifier
  \texttt{single\_file\_demo/\allowbreak meaning\_receipt\_\allowbreak verifier.js},
  exercised
  against the same fixtures in two runtimes, under Node in CI and in the
  browser demo. (\texttt{standalone\_verifier/} verifies the older
