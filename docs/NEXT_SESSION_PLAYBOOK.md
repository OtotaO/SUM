...
- **ISO/IEC 27001:2022 A.8.15 Logging + SOC 2 CC7.2 System Operations** — fourth and fifth regimes, closed by PR #132 (bundled, 2026-05-03). Five rules R1–R5 each (the minimum record-keeping floor). 38 tests (~19 each). Wire specs at `docs/COMPLIANCE_ISO_27001_8_15.md` and `docs/COMPLIANCE_SOC_2_CC_7_2.md`. **Empirical finding worth carrying forward:** there is a *minimum record-keeping floor* common to most record-keeping regimes — five regimes (across EU AI law, EU privacy law, US health law, international standard, US audit-attestation) agree on the floor's five-rule shape even though the statutes differ wildly.

- **PCI DSS v4.0 Requirement 10** — sixth regime, closed by PR #133 (2026-05-03). Six rules R1–R6 (R1–R5 as the minimum record-keeping floor + R6 event-content-completeness with operation-specific anchors). ~25 tests. Wire spec at `docs/COMPLIANCE_PCI_DSS_4_REQ_10.md` — the longest "what this validator does NOT pin" section in the slate, naming 10.1 organisational, 10.2.1.* event-type coverage, 10.2.2 user_id structural gap (load-bearing — `audit_log.v1` does not carry user_id), 10.3 log file protection, 10.4 review process, 10.5 retention, 10.7 failure alerting, and cardholder data inventory as out of scope. **The user_id gap is the truthful finding from the slate:** PCI deployments using SUM as a payment-adjacent component need either a schema extension or an authenticating proxy.

**Six-regime slate complete.** ``sum.compliance_report.v1`` shape held without modification across six statutorily different regimes (EU AI law, EU privacy law, US health law, international ISMS standard, US audit-attestation, payment-card industry standard). The substrate's regime-agnosticism is settled empirical fact. Total compliance suite: 152 tests.
