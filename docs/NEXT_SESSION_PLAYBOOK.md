...
**Future regime still queued (same template).** PCI DSS 4.0 Requirement 10 — remaining record-keeping regime in the slate; planned for a separate PR because Req 10 has 7 sub-requirements (10.1–10.7) and many sub-rules. A more honest "what this does NOT pin" section than the thinner regimes; a separate PR keeps the truthful scope-naming tight.

**Different shape, separate PR family** (out of scope for this priority): HIPAA § 164.514 de-identification (transformation rules), EU AI Act Art 13 / 16 / 50 (transparency / QMS / disclosure).
