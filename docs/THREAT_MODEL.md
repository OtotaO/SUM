...
| Cross-provider cache poisoning on `/api/render` (e.g., caller requests `provider: "openai"` and gets a cached Anthropic-rendered tome whose receipt says `provider: "anthropic"`) | ✅ | v0.6.0 isolates the Worker KV cache namespace per resolved provider when the caller specifies one explicitly — OpenAI and Anthropic renders of the same `(triples, sliders)` tuple live in disjoint cache keys, so a `provider`-mismatched HIT is structurally impossible. Default-provider requests (no `body.provider`) still use the bare key to preserve backwards compatibility with pre-v0.6.0 cached entries; the receipt's `provider` field on the cached value reflects the actual issuer. See `worker/src/routes/render.ts::handleRender` |
| Silent provider substitution on explicit selection | ✅ | v0.6.0 — `POST /api/render` with `{"provider":"openai"}` fails fast (HTTP 503) if `OPENAI_API_KEY` is absent rather than falling back to Anthropic. Receipt's `provider` field would no longer match the caller's intent under silent substitution, so the dispatcher refuses to substitute |
| Local-LLM endpoint port guessing (v0.6.0) | ✅ | `llm_dispatch.get_adapter` for the generic `local:<model>` prefix fails fast with `ValueError` if `SUM_LOCAL_LLM_BASE` is unset, rather than guessing a port across the vLLM / TGI / LM Studio / oobabooga / Ollama / llama.cpp landscape. `ollama:` and `llamacpp:` prefixes use their projects' documented default ports (11434, 8080) — pinned in unit tests |
