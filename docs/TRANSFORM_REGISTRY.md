...
**Status:** shipped. Three transforms are registered **Python-side** (`slider`, `extract`, `compose`), dispatched by the CLI `sum transform apply <name>`. The Worker `POST /api/transform` route currently dispatches **`slider` only** — `extract`/`compose` return HTTP 400 (`unknown transform`) there and are reached through the Python CLI surface; wiring them into the Worker registry is open follow-up. The slider transform's LLM-axis dispatch is wired Python-side (via OpenAI through `OPENAI_API_KEY`); Worker LLM-axis dispatch through this route is the open sibling-PR. Until then, Worker LLM-axis renders go through legacy `POST /api/render` producing `sum.render_receipt.v1`.
