...
class OpenAIProvider(BaseLLMProvider):
    ...
    def _fallback_generate(self, prompt: str) -> str:
        ...
        try:
            ...
            return engine.summarize_text(prompt, method='advanced')
        except:
...
class AnthropicProvider(BaseLLMProvider):
    ...
    def _fallback_generate(self, prompt: str) -> str:
        from summarization_engine import SummarizationEngine
        engine = SummarizationEngine()
        return engine.summarize(prompt, method='advanced')
...
class LocalProvider(BaseLLMProvider):
    ...
    async def generate(self, prompt: str, config: LLMConfig) -> str:
        ...
        try:
            ...
            # Calculate ratio from max_tokens
            ratio = min(config.max_tokens / 1000, 0.5)
            
            return engine.summarize_text(prompt, method=method, ratio=ratio)
        except:
