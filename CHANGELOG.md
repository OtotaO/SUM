...
- **Security: a supplied HMAC key now requires the HMAC signature.** `sum verify --signing-key K` and MCP `verify` / `verify_bind` with `signing_key` reject a bundle that carries no HMAC `signature`, in default and `--strict` modes, matching `CanonicalCodec.import_bundle`'s downgrade protection. Previously an attacker could delete the HMAC, change the content, add their own valid Ed25519 signature, and pass `--strict --signing-key K` with exit code 0. The MCP `signatures.hmac` field reports the new `missing` status. A non-string or non-ASCII `signature` is reported as invalid instead of raising. An explicitly empty key (`--signing-key ""`, MCP `signing_key=""`, typically an unset secret) is rejected as a usage error (CLI exit 2, MCP `schema`) instead of silently falling back to embedded-key Ed25519 verification; the internal helper still treats an empty key as no key, as `CanonicalCodec` does. The defect is present in every published release (the CLI from 0.1.0, the MCP tool from 0.4.1, through 0.10.0) and in the v0.11.0 tag. THREAT_MODEL §2.1 corrected; regression tests fail on the previous code.
- Regenerated the vendored browser verifier bundle (`single_file_demo/vendor/sum-verify-deps.js` and `LICENSE.txt`) for canonicalize 5.1.0 (from 4.0.0; supersedes dependabot #522, which replaced #511, whose CI failed because the bundle was not rebuilt). A differential fuzz of 40,000 documents found 0 byte differences against 4.0.0. Outputs differ only for inputs that are not JSON and for very deep nesting, which 4.0.0 could not canonicalize (stack overflow) and 5.1.0 can. The stated Node floor for the in-page verifier is now 20 (canonicalize 5 uses `String.prototype.isWellFormed`; the package itself declares Node >= 22). The Worker signer stays on canonicalize ^4.0.0; the live site serves the old bundle until the next Worker deploy.
- **Dependency floors reset to tested minimums.** Published 0.10.0 (and the v0.11.0 tag) declare `joserfc>=1.0.0` for `[verify]`, but joserfc 1.0.x and 1.1.0 fail `python -m sum_verify --demo`, and releases below 1.6.8 carry published advisories, including GHSA-wphv-vfrh-23q5 on the detached-payload path SUM uses. The floor is now joserfc 1.6.8 (which itself requires cryptography 45.0.1); that pair passes the SDK and receipt-verifier suites. This lowers main's unreleased floor of 1.7.5 (raised by Dependabot in #505) to the tested minimum. Also: `openai>=1.55.3` (earlier releases raise TypeError with httpx 0.28), `spacy>=3.8.2` (3.8.0 and 3.8.1 are yanked on PyPI), `click>=8.0` restored (Dependabot had raised it to 8.5.0 although the floor job installs 8.0.0), and `pytest-asyncio>=1.3.0` to pair with pytest 9. The floor smoke job now fails if pip resolves anything other than the pinned floors and adds a `[verify]` floor replay. Dependabot uses `increase-if-necessary` for pip, monthly. The Worker's esbuild and miniflare pins match wrangler 4.131.1 again (61 nested packages removed); version updates for wrangler, esbuild and miniflare are now made by hand together (Dependabot alerts and security updates are disabled for this repository, so no automated update PRs cover these three; the Scorecard OSV scan still reports known vulnerabilities), and CI fails if the lockfile nests a second copy. Supersedes dependabot #512 to #515.
...
- Completed a retained-source browser review flow: compare an existing rewrite, inspect linked passages, record decisions, and export/import exact review packets. Generated output enters the same flow. Browser comparison is literal and advisory; source and human decisions remain unsigned. Receipt verification now checks displayed output, selected triples and slider bindings. Input/settings/review changes suppress stale extraction, rendering, verification and export results. Density is applied once. Conservative extraction fallback preserves complete source spans. Added labels, live status and reduced-motion support.
- Corrected provider-funded quota classification (#499). Added bounded streamed requests, validated triples/sliders, configured operator models, provider deadlines and SQLite Durable Object admission: per-IP and global attempt limits plus leased concurrency. Missing paid-admission infrastructure fails closed; canonical routes remain available. Admission limits attempts rather than invoiced dollars; account-level provider spending controls remain separate. New binding/migration is part of Worker deployment.
- Added explicit offline SDK `TrustPolicy` and reports (SDK API 1.2.0), with trusted key-ID/material catalogs, immutable revocation snapshots, revoked-key alias handling, freshness versus archival checks and independently supplied artifact bindings. Legacy default verification APIs and historical receipt bytes remain compatible. Policy does not authenticate an organization, validate sampling, or automatically validate every chain hop; callers apply per-hop policy when required. Added low-level transform revocation parity.
- Made bound agent values immutable snapshots; rejected oversized values before eviction. **Compatibility:** text handles keep their legacy namespace; JSON and byte values use typed v2 handles and must be rebound. There is no ambiguous cross-type legacy alias. MCP extraction now returns a setup error rather than installing a missing model over the network. Chunking retains complete sentences within explicit model/resource caps and rejects unresolved overlong input instead of silently splitting it. `make install` now matches full test dependencies; `install-verify` stays lightweight.
- Unified scalar/readout scoring for empty-source cases. New research receipts default to descriptive scope; a population confidence interpretation requires an issuer-asserted independent sampling contract with a fixed policy and distribution match. Added canonical scorer configuration and ordered source/output manifests, hash checks and NLI token-window inspection metadata. These identify evidence; they do not rederive a model's scores or prove meaning preservation.
- Retired T4's composition-invariance inference. Corrected post-processing of the stored observations shows six worsening endpoint pairs across 86 documents despite unchanged corpus medians. The v2 descriptive artifact, paired rows and erratum are committed. All historical signed JSON remains unchanged, with explicit archival regeneration adapters. Candidate paths no longer imply Pareto optimality or monotone quality. Human validation and production-generation benchmarks remain open.
- Replaced the broken extension/research API client with explicit local capture, copy and a separate workbench link. Chrome, Edge and Firefox development packages are generated from one source; no background collection or text-bearing network calls. Store publication and actual per-browser load testing are separate work. Terminal UI now says Not measured until a historical demo is replayed and discards late results after edits; its unwired action is labelled CLI instructions. Docker remains an unvalidated internal research recipe.
- Added a declared product-surface manifest and refreshed the existing product vision/playbook. Wired Worker admission/input, browser packet/DOM, extension-generation and TUI-state regressions into CI, and extended the deployed-asset byte guard. Unicode codepoint ordering now matches Python for supplementary characters in signed bindings and density selection.
...
- **Verifier hardening from a fresh adversarial audit (2026-07-02).** Five fixes,
  ...
  `canonical_tome` — previously a bundle whose mirror was edited *after* attest
  still verified green (the signature covers the tome, not the mirror) while
  `sum transform apply compose` consumed the edited mirror; (2) `sum verify`
  exits 2 (malformed input), not 1 (tampered), on an unreadable input path,
  matching its documented 0/1/2 contract; (3) new `sum_verify.SumVerifyError`
  common base — `except SumVerifyError` now catches every failure class the
  SDK raises (previously required 4+ classes; `ReceiptVerifyError` read as a
  base but was not one); (4) the HMAC bundle verifier compares signatures as
  bytes — a malformed non-ASCII signature string used to CRASH
  `hmac.compare_digest` with `TypeError` instead of failing verification
  (found by the property-based totality sweep); (5) the T4 drift-composition
  bench runner now applies DKW correctly (quantile-level shift + median
  confidence-interval overlap, with an explicit `n_too_small_to_distinguish`
  verdict) — the previous revision compared a drift-domain median delta
  against a CDF-probability epsilon, which was anti-conservative at small n
  (the committed 2026-05-22 receipt's verdict is unaffected: its max median
  delta was exactly 0.0, invariant under both tests). Bonus: the drift-budget
  `joint_confidence` display clamps at 0 instead of going negative past ~1/δ
  hops. An independent re-derivation audit of the conformal bound kernels
  (Hoeffding / empirical-Bernstein / Clopper–Pearson), the integer-micro wire
  replay, the Bonferroni split, and both committed golden receipts confirmed
